 ...
 * Fix: Valores aleatórios calculados uma vez via useMemo para evitar re-renders infinitos.
 */
export function AnatomiaDigital() {
  const coordinates = useMemo(() => ({
    xAxis: Math.random().toFixed(4),
    yAxis: Math.random().toFixed(4),
  }), []);
  ...
  return (
    <div className="absolute inset-0 z-20 pointer-events-none overflow-hidden">
      ...
      <div className="absolute top-4 right-4 text-[10px] font-mono text-cyan-400/60 space-y-1" aria-hidden="true">
        <div>X_AXIS: {coordinates.xAxis}</div>
        <div>Y_AXIS: {coordinates.yAxis}</div>
