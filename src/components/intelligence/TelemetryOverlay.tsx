...
import { Battery, Signal, Navigation2, Compass, Zap } from 'lucide-react';
...
export function TelemetryOverlay() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setTelemetry(prev => ({
        alt: prev.alt + (Math.random() - 0.5) * 0.2,
    ...
    <div className="absolute inset-0 pointer-events-none z-30 p-4 font-mono text-[10px] text-cyan-400/80">
      ...
      <div className="absolute top-4 left-4 flex gap-4 bg-black/20 backdrop-blur-sm p-2 border border-white/5 rounded">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span>BAT: {telemetry.battery.toFixed(1)}%</span>
        </div>
      </div>
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 border border-white/5 rounded-full opacity-20">
        <motion.div 
